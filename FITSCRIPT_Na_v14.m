...
MCFLAG=1;
MCSTART=1;
savefile='MCMC_LM_1_v14b_quat_testOct06.mat';
...
% just try an initial candidate from fminsearch
Estimates=fminsearch(@(x)Rfit_tot_LM_v14(x,ydata,const,yerr),start,opt);
x = Estimates
[errtot,err,ymodel] = Rfit_tot_LM_v14(x,ydata,const,yerr);
if(MCFLAG)
%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% MC loop                             %%%
...
while( ((sum(accave<ones(1,Nparm).*0.4) || sum(accave>ones(1,Nparm).*0.6))) && MM<50 )
  ...
  for i_sub=1:N_warmup,
    ...
    [Xout, acc, newXbest,errbest,err] = MCMCstep(Xout, yerr, const,h,xmin,xmax, Xbest,ydata,errbest,@Rfit_tot_LM_v14);
    accave = accave + acc;
    Xbest = newXbest;
  end;
